...
export interface DocumentWithAccess {
  ...
  access_type: 'DIRECT' | 'REFERENCED';
  source_policy_id: string;
  reference_id?: string;
  transition_type?: PolicyTransitionType | null;
}
...
export class DocumentAccessService {
  ...
  static async getPolicyDocuments(
    policyId: string,
  ...
  private static async getDocumentsEfficiently(
    policyId: string,
    options: DocumentAccessOptions
  ): Promise<DocumentWithAccess[]> {
    ...
    const directDocs: DocumentWithAccess[] = policyWithDocs.documents.map(doc => ({
      ...doc,
    ...
    const referencedDocs: DocumentWithAccess[] = policyWithDocs.document_references.map(ref => ({
      ...ref.source_document,
      access_type: 'REFERENCED' as const,
  ...
  static clearCache(policyId?: string): void {
    if (policyId) {
  ...
  static async getDocumentAccessStats(policyId: string): Promise<{
    total: number;
    direct: number;
    referenced: number;
